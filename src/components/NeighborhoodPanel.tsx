...
const BarSegment: React.FC<{ label: string; value: number; total: number; color: string }> = ({
  ...
  return (
    <div className="flex items-center gap-2 py-1">
      <div className="w-24 text-xs text-surface-400 flex-shrink-0">{label}</div>
      <div className="flex-1 h-4 bg-surface-800 rounded-full overflow-hidden">
        <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: color }} />
      </div>
      <div className="w-12 text-xs text-surface-300 text-right">{pct.toFixed(0)}%</div>
    </div>
  );
};
...
export const NeighborhoodPanel: React.FC<PanelProps> = ({ data: d, metroAverages: avg, onClose }) => {
  ...
  return (
    <div className="absolute top-0 left-0 z-20 h-full w-[380px] max-w-[90vw] overflow-y-auto
      ...
      {/* Header */}
      <div className="sticky top-0 bg-surface-950/95 backdrop-blur-xl border-b border-surface-800/50 px-6 py-5">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-xl font-display font-bold text-white">{d.nimi}</h2>
            <p className="text-surface-400 text-sm mt-0.5">{d.pno}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={item.label} className="bg-surface-900/60 rounded-lg p-3">
