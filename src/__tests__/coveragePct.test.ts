...
describe('getCoveragePct', () => {
  it('returns the real coverage for a metric present in the map', () => {
    ...
    expect(getCoveragePct('transit_stop_density')).toBe(10.9);
    expect(getCoveragePct('property_price_sqm')).toBe(30.3);
  });
...
describe('isPartialCoverage', () => {
  it('flags sparse layers below the partial threshold', () => {
    expect(isPartialCoverage('transit_stop_density')).toBe(true); // 10.9
    expect(isPartialCoverage('property_price_sqm')).toBe(true); // 30.3
  });
...
describe('isLowCoverage', () => {
  it('flags only genuinely sparse layers (below the low threshold)', () => {
    ...
    expect(isLowCoverage('rental_price_sqm')).toBe(true); // 15.7
    expect(isLowCoverage('property_price_sqm')).toBe(true); // 30.3
  });
