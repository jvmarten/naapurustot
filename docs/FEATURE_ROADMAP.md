...
> Generated 2026-07-17. **Supersedes the 2026-06-17 roadmap.** From that plan, national filtering (`?city=all` search over all 3,018 areas), accent-folded + municipality search, the prerendered `/vertaa/` X-vs-Y comparison pages, and the server route-test gate all shipped — alongside a month of un-roadmapped work (voting layers, foreign-language time series, fit-for-you score, profile SEO/perf, mobile fixes). The isochrone feature was disabled (`ISOCHRONE_ENABLED`), mooting its parity items. The remaining candidates were re-triaged for impact and **capped at three items per category** — 9 items total. Item IDs are fresh and do not map to the prior roadmap's.

## The headline

Three fronts, in priority order:

1. **The two most decision-relevant missing data layers**, both real-sourced, postal-or-finer, zero data-bundle cost: low-income share / segregation (`hr_pi_tul`) and distance-to-essential-services including the missing **pharmacy** category — plus job self-sufficiency as a near-free quick win from already-fetched Paavo fields.
2. **Fixing what users already see** — mobile users can never reach a comparison chart, the radar misrepresents rural areas with hardcoded metro-centric axes, and "where does my area rank?" requires scanning ~200 rows.
3. **Making the safety gates real** — pushed data regenerations skip full validation, the coverage ratchet is provably inert, and the core docs state a bundle budget that is 33 KB stale.

## The bundle-budget reality (read before implementing anything)

CI fails when the gzipped sum of **all** app JS (lazy chunks included, only `maplibre-*` excluded) exceeds the `BUDGET` constant in `scripts/check-bundle-size.mjs` — currently **313,000 bytes** (CLAUDE.md and ARCHITECTURE.md still say 280,000; PO-2 fixes them). Measure headroom with `npm run bundle:check` before every push. `fi.json` is statically bundled (not free); `en.json`/`sv.json` are lazy `?url` assets (free). Build-time / prerender / `?url`-asset surfaces cost zero bundle bytes — both data layers below ship their values that way. **Exactly one designated item per batch may raise `BUDGET`** (history 256→280→…→313).
...
**Value-pruned in this rewrite (real but outside the 3-per-category cap; fair game later):** Ryhti building-permit construction-flow upgrade of `construction_activity` (the permit register *is* populated today — strongest next data-layer candidate), SYKE protected-nature access layer, React profile-page enrichment parity with its own `<noscript>`, Wikidata `sameAs` entity linking, libraries layer (Kirkanta v4), cultural-heritage layer (Museovirasto WFS), FMI 1 km microclimate grid, fixing the no-op `tsc --noEmit` CI step, stopping `dist/stats.html` from shipping publicly, SplitMapView planning-overlay parity, Speculation Rules prefetch, adaptive `theme-color`, onboarding-tour SR announcements, CSV/PDF export completeness, RegionRankingTable export parity, forced-colors CSS, legend no-data swatch, `<main>` landmark + language-switch announcements, filter-preset login-merge guard/tombstones, visual-regression keep-or-kill decision, DCAT/GeoJSON open-data packaging.
...
### QW-3 Re-baseline the toothless coverage ratchet
...
### CF-3 ComparisonPanel: multi-series radar + mobile chart parity + winner summary
...
| **Why** | Metro-centric caps peg most of the 3,018 areas near the floor, so the radar shape misrepresents rural areas — and the constants go stale on every data refresh. A data-honesty fix and the foundation for CF-3's N-series radar. |
...
| **Why** | Today a hand-edited fetch script + regenerated GeoJSON merges with no sanity validation — a silently nulled column would ship. Closes a real integrity hole and protects both data-layer items in this roadmap; merge it first. |
...
**Parallel-safety:** no two items share a file. CF-1 is the sole layer-registry editor, `build:data` runner, locale-adder, and BUDGET-bumper. IN-1 owns `auto-merge.yml` + `validate_data.py`; PO-1 owns `RadarChart.tsx`; QW-2 owns `App.tsx` + the ranking tables. Merge IN-1 before CF-1 so the new gate validates it.

### Batch 2 — Fifteen-minute access layers + comparison upgrade

| Item | Title | Category | Complexity |
|---|---|---|---|
| CF-2 | Distance-to-essential-services layers (+ pharmacy) | Core | Large |
| CF-3 | Multi-series radar + mobile chart parity | Core | Medium |

**Parallel-safety:** CF-2 is the sole layer-registry editor and `build:data` runner and the **sole BUDGET-bumper** — size its raise to also cover CF-3's ~1.5 KB (CF-3 must not edit `check-bundle-size.mjs`). CF-3 owns `RadarChart.tsx` (PO-1 merged) + `ComparisonPanel.tsx` — no file overlap with CF-2. Both add locale keys — distinct keys, stagger merges.

### Batch 3 — Last-layer quick win + final truth-up

| Item | Title | Category | Complexity |
|---|---|---|---|
| QW-1 | Job self-sufficiency layer | Quick Win | Small |
| PO-2 | Doc truth-up (budget/counts) | Polish | Small |
| QW-3 | Re-baseline the coverage ratchet | Quick Win | Small |

**Parallel-safety:** QW-1 is the sole layer-registry editor, `build:data` runner, locale-adder, and BUDGET-bumper. PO-2 and QW-3 are deliberately **last** — run them only after everything else merges so the documented budget, layer counts, and measured coverage reflect the finished roadmap.

---

### Method note

Statuses verified against the live codebase on 2026-07-17 (git log since 2026-06-17, grep/file checks for each carried-over item): shipped items dropped, the isochrone-dependent item mooted, and the remaining candidates re-ranked by decision-relevance per byte of complexity, then capped at three per category; the pruned tail is listed under "value-pruned" above and remains fair game.
