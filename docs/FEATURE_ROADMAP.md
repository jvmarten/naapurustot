...
> **Implementation status (2026-06-08):** All 36 build-now items below are implemented on branch `claude/complete-feature-roadmap` (Batches 1–5; see git history). The deeper-granularity data layers (demographic grid, OSM footprints, MML elevation), the commute/isochrone destination filter, and the backend-ops/CI/SEO-ping infrastructure items were dropped per owner request before implementation.
