...
### Region switching and the multi-region view

In a region view, a click that hits no postal area is resolved to the seutukunta under it by `regionAt()` (`utils/regionHit.ts`) — a synchronous point-in-polygon over the same parsed `seutukunnat.topojson` the boundary line draws from, so no hit-test layer is added to the style. App then offers "Vaihda / Näytä molemmat" (switch / show both) in a small prompt (`components/RegionSwitch.tsx`): top-centre on desktop, in the peek bar's bottom slot on narrow touch screens. The click is skipped in draw/select mode, on a planning feature, and within the touch-ring distance of a displayed postal area (the outlines are generalised to 1:1,000,000, so a click in a region's own bay can land in a neighbour's outline).

Region state is `useReducer(regionReducer)` over `{ city, extra }` (`utils/regionSet.ts`): `city` is the primary region, `extra` the regions shown alongside it. Dispatching a bare CityFilter is a plain switch that drops the extras, so every older switch path (search, deep link, geolocation, favorites, CitySelector) keeps its single-region meaning; `{ add }` / `{ remove }` change the set. `useMapData(city, { extra })` loads each region through the per-region cache and merges them (`mergeRegionData`; postal codes are unique nationally, so `promoteId: 'pno'` is safe) with averages recomputed over the union — never averaged averages. The URL carries the set as `city=a,b`. With several regions on the map: an add loads the region *before* committing it (a failed fetch must not error the merged view); comparison scope is forced to national, since "within this region" has no single region to mean; grids are disabled (`useGridData(…, enabled)`), because a grid covers only the primary's shard yet its zoom crossfade fades the whole postal fill; the price region-estimate tint is off; and panel / peek / tooltip "vs. seutu" baselines use each area's own region (`averagesByRegion`). The quality-recompute effect keys on the whole displayed set (`regionKey`), since loading a region re-points the global quality-scale cohort at that region alone.
