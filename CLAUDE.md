...
17. **`loadHeightField` spends the whole tile budget.** `terrainZoomFor` targets ~2 levels below the camera, which is right for a shadow's soft EDGE and wrong for its CASTER: at a continental camera that picked ~4.6 km cells, and Finnish relief is 50–200 m over a few km, so the hills were averaged flat before the sweep saw them and the sunlit half of the map had no relief on it at all. The sweep's cost is bounded by `MAX_TILES × tileSize²` — by the tile COUNT, not the level — so after stepping *down* to fit the budget it steps back *up* while a finer level still fits. That is free resolution, and it cannot exceed the pyramid's top, which is the level the street zooms were tuned on.
