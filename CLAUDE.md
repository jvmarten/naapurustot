...
7. Fetches are padded past the viewport (`fetchPadMetres`) so off-screen buildings still cast into view, and the loaded bbox + plan are cached so a zoom-in or short pan reuses them. Padding is paid for on all four sides — a 15 % margin is ~35 % more query area.
