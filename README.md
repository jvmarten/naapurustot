...
```
src/
├── main.tsx                 # Entry point: React root + optional Sentry init
...
│   ├── SettingsDropdown.tsx # Theme, language, opacity controls
│   └── ToolsDropdown.tsx    # Filter/ranking/wizard toggle buttons
├── hooks/
│   ├── useMapData.ts        # Fetch + process TopoJSON (eager prefetch)
│   ├── useSelectedNeighborhood.ts  # Selected + pinned (max 3)
│   ├── useTheme.tsx         # Dark/light/system theme context
│   ├── useUrlState.ts       # URL ↔ state sync (?pno=, ?layer=, ?compare=)
│   ├── useGridData.ts       # Lazy-load fine-grained grid data (250m cells)
│   ├── useFavorites.ts      # localStorage-backed favorites list
...
│   ├── scoreCard.ts         # PNG score card via html-to-image
...
├── data/
│   └── metro_neighborhoods.topojson  # Static dataset loaded by the app
└── __tests__/               # 30+ Vitest test files

scripts/                     # Python data pipeline
├── prepare_data.py          # Main pipeline: fetches, merges, outputs GeoJSON
├── fetch_*.py               # API-specific data fetchers
├── *.json                   # Pre-computed intermediate data files
└── build_grid_data.mjs      # TopoJSON grid generation (Node.js)

public/data/
├── metro_neighborhoods.geojson   # Source of truth (~1.3 MB, ~160 neighborhoods)
└── light_pollution_grid.geojson  # 250m grid cells for light pollution layer
```
...
|----------|---------|-------------|
| `ci.yml` | Push/PR to main | Lint → type check → test → build → E2E → bundle size check (250 KB gzip JS budget) |
...
| `auto-merge.yml` | On PR approval | Auto-merge approved PRs |
